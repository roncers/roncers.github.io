...
import { MISC_TABS } from "@/types/available-tabs/tabs-index.types"
import { withTabStructure } from "../hocs/tabWithTable"

function MyProjectsContent(_: TabComponentProps) {
  const { t } = useTranslation()
  const links = Object.values(MISC_TABS).map((tab) => ({
    label: t(tab.i18key),
    ...tab,
  }))
  ...
  return (
    ...
    >
      <section data-name="intro">
        <h2 className="default-header-1">{t("info.miscellaneous.title")}</h2>
        <p className="default-description mt-1">{t("info.miscellaneous.description")}</p>
      </section>
